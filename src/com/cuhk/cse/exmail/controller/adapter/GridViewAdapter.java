package com.cuhk.cse.exmail.controller.adapter;
...
import com.cuhk.cse.exmail.controller.utils.AttachmentUtil;
...
public class GridViewAdapter<T> extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		holder.textView2.setText(AttachmentUtil.convertStorage(affInfos
				.getFilesize()));
	...
	private String[] MUSIC_EXTS = new String[] { "mp3", "wma", "wav", "ogg",
			"ape", "acc", "amr" };
	...
	public Bitmap getCategoryFromPath(String path) {
		...
		if (ext.equalsIgnoreCase(PDF_EXTS)) {
			return bitmap = BitmapFactory.decodeResource(
					mActivity.getResources(), R.drawable.pdf);
		}
		if (matchExts(ext, DOC_EXTS)) {
			return bitmap = BitmapFactory.decodeResource(
					mActivity.getResources(), R.drawable.doc);
		}
		if (matchExts(ext, PPT_EXTS)) {
			return bitmap = BitmapFactory.decodeResource(
					mActivity.getResources(), R.drawable.ppt);
		}
		if (matchExts(ext, XSL_EXTS)) {
			return bitmap = BitmapFactory.decodeResource(
					mActivity.getResources(), R.drawable.xls);
		}
		if (matchExts(ext, TXT_EXTS)) {
			return bitmap = BitmapFactory.decodeResource(
					mActivity.getResources(), R.drawable.file_doc);
		}

		if (matchExts(ext, ZIP_EXTS)) {
			return bitmap = BitmapFactory.decodeResource(
					mActivity.getResources(), R.drawable.file_archive);
		}

		if (matchExts(ext, VIDEO_EXTS)) {
			return bitmap = BitmapFactory.decodeResource(
					mActivity.getResources(), R.drawable.file_video);
		}
		if (matchExts(ext, MUSIC_EXTS)) {
			return bitmap = BitmapFactory.decodeResource(
					mActivity.getResources(), R.drawable.file_audio);
		}
