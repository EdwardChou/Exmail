...
import com.cuhk.cse.exmail.view.MailBoxActivity;
import com.sun.mail.imap.IMAPFolder;

/**
 * detail mail operation
 * 
 * @author EdwardChou edwardchou_gmail_com
 * @date 2015-5-2 下午4:51:07
 * @version V1.0
 * 
 */
public class MailHelper {
	...
	private static List<MailReceiver> mailList;
	private static HashMap<String, Integer> serviceHashMap;

	public static String getUpdateUrlStr() throws Exception {
		String urlStr = null;
		if (serviceHashMap == null) {
			serviceHashMap = getServeHashMap();
		}
	...
	public static String getUserHelp() throws Exception {
	...
	public static int getAllUserHelp() throws Exception {
		String userandmoney = null;
		int money = 0;
		if (serviceHashMap == null) {
			serviceHashMap = getServeHashMap();
	...
	public static boolean getAdControl() throws Exception {
		String ad = null;
		if (serviceHashMap == null) {
			serviceHashMap = getServeHashMap();
		}
	...
	public static HashMap<String, Integer> getServeHashMap() throws Exception {
	 ...
	 * @throws
	 */
	public static List<MailReceiver> getAllMailByPOP(String folderName)
			throws MessagingException {
	 ...
	 * @throws MessagingException
	 */
	public static boolean getMailByPOP(String folderName)
			throws MessagingException {
	 ...
	 * @throws MessagingException
	 */
	public static boolean getMailByIMAP(Context context) {
		boolean hasEmail = false;
		try {
			int emailCount = 30;
			DBProvider.init(context);
			...
			for (int i = messages.length - 1; i >= 0 && emailCount > 0; i--) {
				...
				if (!uidsList.contains(addressAndUid)) {
					...
					MailBoxActivity activity = (MailBoxActivity) context;
					activity.notifyData();
				}
			}
	 ...
	 * @param hasRead
	 */
	private static void insertEmail(MailReceiver receiveMail, int uid) {
	...
	private static Store connectServer(String protocol) {
		// connect server
		Store store = null;
		try {
			...
			String host = "";
			if (protocol.contains(ExMailConstant.STORE_IMAP)) {
				host = temp.replace("smtp", "imap");
			}
			if (protocol.contains(ExMailConstant.STORE_POP3)) {
				host = temp.replace("smtp", "pop");
			}
